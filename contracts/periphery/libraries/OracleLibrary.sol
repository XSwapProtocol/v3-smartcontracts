...
library OracleLibrary {
    /// @notice Calculates time-weighted means of tick and liquidity for a given XSwap pool
        ...
        returns (int24 arithmeticMeanTick, uint128 harmonicMeanLiquidity)
    {
        ...
        ) = IXSwapPool(pool).observe(secondsAgos);
    ...
    /// @param pool Address of XSwap pool that we want to observe
    ...
    function getOldestObservationSecondsAgo(
        ...
        ) = IXSwapPool(pool).slot0();
        require(observationCardinality > 0, "NI");

        (uint32 observationTimestamp, , , bool initialized) = IXSwapPool(pool)
            .observations((observationIndex + 1) % observationCardinality);

        // The next index might not be initialized if the cardinality is in the process of increasing
        // In this case the oldest observation is always in index 0
        if (!initialized) {
            (observationTimestamp, , , ) = IXSwapPool(pool).observations(0);
        }
    ...
    /// @param pool Address of XSwap pool
    ...
    function getBlockStartingTickAndLiquidity(
        ...
        if (observationTimestamp != uint32(block.timestamp)) {
            return (tick, IXSwapPool(pool).liquidity());
        }
        ...
        ) = IXSwapPool(pool).observations(prevIndex);
