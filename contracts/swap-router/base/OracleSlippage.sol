...
import "../../periphery/base/BlockTimestamp.sol";
    ...
    BlockTimestamp
{
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (IXSwapPool pool) {
        pool = IXSwapPool(
        ...
        returns (int256 syntheticAverageTick, int256 syntheticCurrentTick)
    {
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            (address tokenIn, address tokenOut, uint24 fee) = path
                .decodeFirstPool();
            IXSwapPool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = IXSwapPool(pool).slot0();
            }
